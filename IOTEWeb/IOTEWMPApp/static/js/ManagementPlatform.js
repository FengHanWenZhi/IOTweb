...
simulateFloodlightSensor = function () {
    ...
    $.get("/floodlightMonitor/", {read: "sensorRead"}, function (data) {
        ...
        for (var i = 0; i < workingSensors.length; i++) {
            console.info("simulateFloodlightSensor");
            newSensor = workingSensors[i];
            newSensor.luminance = 30 + Math.round(50*Math.random()*100)/100; //有效数字精确到小数点后两位
...
simulateTemperatureSensor = function () {
    ...
    $.get("/temperatureMonitor/", {read: "sensorRead"}, function (data) {
        ...
        //随机改变检测的数据
        for (var i = 0; i < workingSensors.length; i++) {
            console.info("simulateTemperatureSensor");
            newSensor = workingSensors[i];
            newSensor.temperature = 5 + Math.round(50*Math.random()*100)/100; //有效数字精确到小数点后两位
...
simulateHumiditySensor = function () {
    ...
    $.get("/humidityMonitor/", {read: "sensorRead"}, function (data) {
        ...
        //随机改变检测的数据
        for (var i = 0; i < workingSensors.length; i++) {
            console.info("simulateHumiditySensor");
            newSensor = workingSensors[i];
            newSensor.humidity = 5 + Math.round(50*Math.random()*100)/100; //有效数字精确到小数点后两位
...
reloadFloodlightTable = function () {
    console.info("reloadFloodlightTable");
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.allSensorsJSON.length; i++) {
                ...
                htmlstr = htmlstr +
                    ...
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].num + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].name + "</td>" +

                    "<td class='text-center'>" +
                    "<a type='button' class='btn btn-xs btn-success btnWork'>"+ deviceStatusString + "</a>" +"</td>"+

                    "<td style='text-align:center'>" + data.allSensorsJSON[i].luminance + " lm" + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].updata + "</td>" +
                    "<td style='text-align:center'>" + data.allSensorsJSON[i].downdata + "</td>" +
...
reloadTemperatureTable = function () {
    console.info("reloadTemperatureTable");
    $.ajax({
...
reloadHumidityTable = function () {
    console.info("reloadHumidityTable");
    $.ajax({
        url: "/humidityMonitor/",
        data: {read: "webRead"},
        type: "GET",
...
$(document).ready(function () {
    ...
    $("#monitorTable").on('click', ".btnEdit", function () {
        ...
        $('#form')[0].reset();
        $("#formBody").show();
        $("#controlDiv2").show();//显示控制选项
        $("#controlDiv").hide();
        $('#controlDiv1').hide();
        $('[name="id"]').val($(this).parent("td").siblings("td.hidden").text());//返回id后面的所有值siblings

        //把deviceStatus的值填入表单，及根据deviceStatus的值显示不同的按钮样式，文字
        // $('[name="deviceStatus"]').val(deviceStatus);
        $('[name="sensorNum"]').val($(this).parent("td").prev().prev().prev().prev().prev().prev().text());
        $('[name="sensorName"]').val($(this).parent("td").prev().prev().prev().prev().prev().text());
        $('[name="sensorUpdata"]').val($(this).parent("td").prev().prev().text());
        $('[name="sensorDowndata"]').val($(this).parent("td").prev().text());
    ...
    $("#btnSaveUp").click(function () {
        ...
        if ("floodlight" == showType)
            ...
            $.ajax({
                ...
                dataType: "JSON",
                success: function (data) {
                    if ("设备号不能修改" == data.renewStatus){
                        alert(data.renewStatus);
                    }else{
                        //如果成功，隐藏弹出框并重新加载数据
                        $('#modal_form').modal('hide');
                        if ("floodlight" == showType)
